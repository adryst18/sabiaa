...
JicamaAnimada({required this.posicion});
}
...
class _TrazoIMinusculaScreenState extends State<TrazoIMinusculaScreen> with TickerProviderStateMixin {
  ...
  void _generarReferencias() {
    _puntos.clear();
    // Línea corta recta vertical
    for (double y = 210; y <= 340; y += 12) { _puntos.add(Offset(165, y)); }
    // Punto flotante superior
    _puntos.add(const Offset(165, 145));
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: const Color(0xFF134074), title: const Text('SABIA - Letra i minúscula', style: TextStyle(color: Colors.white))),
      body: Column(
        children: [
          Expanded(
            child: Center(
                ...
                child: GestureDetector(
                  ...
                  onPanUpdate: (d) {
                    ...
                    setState(() {
                      ...
                      if (_trazoActual.length % 4 == 0) _huerto.add(JicamaAnimada(posicion: d.localPosition));
                    });
                  },
                  onPanEnd: (_) {
                    _audioPlayer.stop();
                  ...
                  child: AnimatedBuilder(
                    ...
                    builder: (context, child) => CustomPaint(
                      painter: LetraIMinusculaPainter(_trazos, _trazoActual, _huerto, _puntos, _cubiertos, _blink.value),
                      size: Size.infinite,
                    ),
                  ),
                ),
  ...
  void _mostrarDialogo() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('¡Fantástico!', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          Row(
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (context) => const TrazoOMayusculaScreen())); },
...
class LetraIMinusculaPainter extends CustomPainter {
  final List<List<Offset>> trazos; final List<Offset> trazoActual; final List<JicamaAnimada> huerto;
  final List<Offset> puntos; final List<bool> cubiertos; final double blink;
  LetraIMinusculaPainter(this.trazos, this.trazoActual, this.huerto, this.puntos, this.cubiertos, this.blink);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    canvas.drawLine(const Offset(165, 210), const Offset(165, 340), sPaint);
    canvas.drawCircle(const Offset(165, 145), 12.0, sPaint);

    canvas.drawLine(const Offset(165, 210), const Offset(165, 340), lPaint);
    canvas.drawCircle(const Offset(165, 145), 4.0, lPaint);
