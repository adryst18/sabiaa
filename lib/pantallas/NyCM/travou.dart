...
class _TrazoUMayusculaScreenState extends State<TrazoUMayusculaScreen> with TickerProviderStateMixin {
  ...
  List<MelonAnimado> _huertoMelones = [];
  ...
  late AnimationController _loopMelonesController;
  ...
  void initState() {
    ...
    _loopMelonesController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 16),
    )..addListener(() {
        setState(() {
          for (var melon in _huertoMelones) {
            if (melon.escalaActual < melon.escalaObjetivo) {
              melon.escalaActual += 0.08;
              if (melon.escalaActual > melon.escalaObjetivo) {
                melon.escalaActual = melon.escalaObjetivo;
              }
            }
          }
        });
      });
    _loopMelonesController.repeat();
  ...
  void dispose() {
    ...
    _loopMelonesController.dispose();
  ...
  void _generarPuntosReferenciaPorPartes() {
    ...
    for (double y = 110; y <= 250; y += 12) {
      _puntosReferenciaIzquierdo.add(Offset(100, y));
    }
...
// 2. Curva inferior (de izquierda a derecha) - CORREGIDO
    ...
    double y = 250 + 65 * math.sin(a);  // CAMBIAR: + por -
        _puntosReferenciaCurva.add(Offset(x, y));
      }

          // 3. Palito derecho (de abajo hacia arriba)
          for (double y = 250; y >= 110; y -= 12) {
            _puntosReferenciaDerecho.add(Offset(230, y));
          }
  ...
  void _evaluarPrecisionMuestraContinua(Offset posicionTractor) {
    ...
    if (_faseTrazoActual == 0) {
      ...
      if (!_cubiertosCurva.contains(false)) {
        ...
        _voiceService.hablar('¡Excelente! Ahora sube por el lado derecho.');
      }
  ...
  void _ejecutarCalificacionYVentana() {
    ...
    _voiceService.hablar('¡Felicidades, lograste escribir la U mayúscula!');
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const TrazoUMinusculaScreen()),
                  );
                },
...
class _MontessoriPainter extends CustomPainter {
  ...
  final List<MelonAnimado> huerto;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final base = Path()
      ..moveTo(100, 110)
      ..lineTo(100, 250)
      ..arcTo(Rect.fromCircle(center: const Offset(165, 250), radius: 65), math.pi, math.pi, false)
      ..lineTo(230, 110);
    ...
    for (var melon in huerto) {
      if (melon.escalaActual > 0.05) {
        canvas.save();
        canvas.translate(melon.posicion.dx, melon.posicion.dy);
        canvas.rotate(melon.rotacion);
        canvas.scale(melon.escalaActual);
