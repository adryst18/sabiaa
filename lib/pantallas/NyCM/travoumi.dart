...
class SandiaAnimada {
  ...
  SandiaAnimada({required this.posicion});
}
...
class _TrazoUMinusculaScreenState extends State<TrazoUMinusculaScreen> with TickerProviderStateMixin {
  ...
  void _generarReferencias() {
    _puntos.clear();
    // Curva de la u minúscula
    for (double y = 200; y <= 270; y += 12) { _puntos.add(Offset(110, y)); }
    for (double a = math.pi; a <= 2 * math.pi; a += 0.25) {
      double x = 155 + 45 * math.cos(a);
      double y = 270 + 45 * math.sin(a);
      _puntos.add(Offset(x, y));
    }
    for (double y = 270; y >= 200; y -= 12) { _puntos.add(Offset(200, y)); }
    // Palito de soporte derecho
    for (double y = 200; y <= 315; y += 12) { _puntos.add(Offset(200, y)); }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: const Color(0xFF134074), title: const Text('SABIA - Letra u minúscula', style: TextStyle(color: Colors.white))),
      body: Column(
        children: [
          Expanded(
            child: Center(
                ...
                child: GestureDetector(
                  ...
                  onPanUpdate: (d) {
                    ...
                    setState(() {
                      ...
                      if (_trazoActual.length % 4 == 0) _huerto.add(SandiaAnimada(posicion: d.localPosition));
                    });
                  },
                  onPanEnd: (_) {
                    _audioPlayer.stop();
                  ...
                  child: AnimatedBuilder(
                    ...
                    builder: (context, child) => CustomPaint(
                      painter: LetraUMinusculaPainter(_trazos, _trazoActual, _huerto, _puntos, _cubiertos, _blink.value),
                      size: Size.infinite,
                    ),
                  ),
                ),
  ...
  void _mostrarDialogo() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('¡Completaste las Vocales!', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF134074))),
        content: Text('¡Felicidades! Lograste una precisión del ${_precision.toStringAsFixed(0)}% en este último ejercicio.', textAlign: TextAlign.center, style: const TextStyle(fontSize: 16)),
        actions: [
          Row(
...
class LetraUMinusculaPainter extends CustomPainter {
  final List<List<Offset>> trazos; final List<Offset> trazoActual; final List<SandiaAnimada> huerto;
  final List<Offset> puntos; final List<bool> cubiertos; final double blink;
  LetraUMinusculaPainter(this.trazos, this.trazoActual, this.huerto, this.puntos, this.cubiertos, this.blink);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final base = Path()..moveTo(110, 200)..lineTo(110, 270)..arcTo(Rect.fromCircle(center: const Offset(155, 270), radius: 45), math.pi, -math.pi, false)..lineTo(200, 200)..moveTo(200, 200)..lineTo(200, 315);
