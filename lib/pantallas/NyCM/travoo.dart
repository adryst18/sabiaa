...
class _TrazoOMayusculaScreenState extends State<TrazoOMayusculaScreen> with TickerProviderStateMixin {
  ...
  List<CalabazaAnimada> _huertoCalabazas = [];
  ...
  late AnimationController _loopCalabazasController;
  ...
  void initState() {
    ...
    _loopCalabazasController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 16),
    )..addListener(() {
        setState(() {
          for (var calabaza in _huertoCalabazas) {
            if (calabaza.escalaActual < calabaza.escalaObjetivo) {
              calabaza.escalaActual += 0.08;
              if (calabaza.escalaActual > calabaza.escalaObjetivo) {
                calabaza.escalaActual = calabaza.escalaObjetivo;
              }
            }
          }
        });
      });
    _loopCalabazasController.repeat();
  ...
  void dispose() {
    ...
    _loopCalabazasController.dispose();
  ...
  void _ejecutarCalificacionYVentana() {
    ...
    _voiceService.hablar('¡Felicidades, lograste escribir la O mayúscula!');
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const TrazoOMinusculaScreen()),
                  );
                },
...
class _MontessoriPainter extends CustomPainter {
  ...
  final List<CalabazaAnimada> huerto;
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Dibujar círculo de la O
    canvas.drawCircle(const Offset(165, 230), 90.0, shadowPaint);
    canvas.drawCircle(const Offset(165, 230), 90.0, letterPaint);
    ...
    for (var calabaza in huerto) {
      if (calabaza.escalaActual > 0.05) {
        canvas.save();
        canvas.translate(calabaza.posicion.dx, calabaza.posicion.dy);
        canvas.rotate(calabaza.rotacion);
        canvas.scale(calabaza.escalaActual);
