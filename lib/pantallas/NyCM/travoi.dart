...
class TrazoIMayusculaScreen extends StatefulWidget {
  const TrazoIMayusculaScreen({super.key});

  @override
  State<TrazoIMayusculaScreen> createState() => _TrazoIMayusculaScreenState();
}

class _TrazoIMayusculaScreenState extends State<TrazoIMayusculaScreen> with TickerProviderStateMixin {
  ...
  late AnimationController _loopPapasController;
  
  int _faseTrazoActual = 0;
  
  List<Offset> _puntosReferenciaTecho = [];
  List<Offset> _puntosReferenciaCuerpo = [];
  List<Offset> _puntosReferenciaBase = [];
  
  List<bool> _cubiertosTecho = [];
  List<bool> _cubiertosCuerpo = [];
  List<bool> _cubiertosBase = [];
  ...
  void initState() {
    ...
    _loopPapasController = AnimationController(
      vsync: this,
      duration: const Duration(milliseconds: 16),
    )..addListener(() {
        setState(() {
          for (var papa in _huertoPapas) {
            if (papa.escalaActual < papa.escalaObjetivo) {
              papa.escalaActual += 0.08;
              if (papa.escalaActual > papa.escalaObjetivo) {
                papa.escalaActual = papa.escalaObjetivo;
              }
            }
          }
        });
      });
    _loopPapasController.repeat();

    _voiceService.hablar('¡Vamos con la letra I mayúscula! Haz la línea de arriba, luego baja por el centro.');
  }

  @override
  void dispose() {
    _semillasBlinkController.dispose();
    _loopPapasController.dispose();
  ...
  void _generarPuntosReferenciaPorPartes() {
    _puntosReferenciaTecho.clear();
    _puntosReferenciaCuerpo.clear();
    _puntosReferenciaBase.clear();

    // 1. Línea del techo
    for (double x = 110; x <= 230; x += 12) {
      _puntosReferenciaTecho.add(Offset(x, 110));
    }

    // 2. Línea del cuerpo (vertical)
    for (double y = 122; y <= 330; y += 12) {
      _puntosReferenciaCuerpo.add(Offset(170, y));
    }

    // 3. Línea de la base
    for (double x = 110; x <= 230; x += 12) {
      _puntosReferenciaBase.add(Offset(x, 340));
    }

    _cubiertosTecho = List.filled(_puntosReferenciaTecho.length, false);
    _cubiertosCuerpo = List.filled(_puntosReferenciaCuerpo.length, false);
    _cubiertosBase = List.filled(_puntosReferenciaBase.length, false);
    
    _faseTrazoActual = 0; 
    _posicionTractor = _puntosReferenciaTecho.first;
  ...
  void _evaluarPrecisionMuestraContinua(Offset posicionTractor) {
    ...
    for (var p in _puntosReferenciaTecho) {
      double d = (posicionTractor - p).distance;
      if (d < menorDistanciaALineaCentro) menorDistanciaALineaCentro = d;
    }
    for (var p in _puntosReferenciaCuerpo) {
      double d = (posicionTractor - p).distance;
      if (d < menorDistanciaALineaCentro) menorDistanciaALineaCentro = d;
    }
    for (var p in _puntosReferenciaBase) {
      double d = (posicionTractor - p).distance;
      if (d < menorDistanciaALineaCentro) menorDistanciaALineaCentro = d;
    }
    ...
    if (_faseTrazoActual == 0) {
      for (int i = 0; i < _puntosReferenciaTecho.length; i++) {
        if ((posicionTractor - _puntosReferenciaTecho[i]).distance < maxCanalSombreado) {
          _cubiertosTecho[i] = true;
        }
      }
      if (!_cubiertosTecho.contains(false)) {
        setState(() => _faseTrazoActual = 1); 
        _voiceService.hablar('¡Perfecto! Ahora baja por el centro.');
      }
    } else if (_faseTrazoActual == 1) {
      for (int i = 0; i < _puntosReferenciaCuerpo.length; i++) {
        if ((posicionTractor - _puntosReferenciaCuerpo[i]).distance < maxCanalSombreado) {
          _cubiertosCuerpo[i] = true;
        }
      }
      if (!_cubiertosCuerpo.contains(false)) {
        setState(() => _faseTrazoActual = 2); 
        _voiceService.hablar('¡Excelente! Ahora haz la línea de la base.');
      }
    } else if (_faseTrazoActual == 2) {
      for (int i = 0; i < _puntosReferenciaBase.length; i++) {
        if ((posicionTractor - _puntosReferenciaBase[i]).distance < maxCanalSombreado) {
          _cubiertosBase[i] = true;
        }
      }
    }
  ...
  void _ejecutarCalificacionYVentana() {
    ...
    _voiceService.hablar('¡Felicidades, lograste escribir la I mayúscula!');
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () {
                  ...
                  Navigator.push(
                    context,
                    MaterialPageRoute(builder: (context) => const TrazoIMinusculaScreen()),
                  );
                },
                          ...
                          builder: (context, child) {
                            return CustomPaint(
                              painter: _MontessoriPainter(
                                ...
                                huerto: _huertoPapas,
                                refTecho: _puntosReferenciaTecho,
                                refCuerpo: _puntosReferenciaCuerpo,
                                refBase: _puntosReferenciaBase,
                                cubiertosTecho: _cubiertosTecho,
                                cubiertosCuerpo: _cubiertosCuerpo,
                                cubiertosBase: _cubiertosBase,
...
class _MontessoriPainter extends CustomPainter {
  ...
  final List<PapaAnimada> huerto;
  
  final List<Offset> refTecho;
  final List<Offset> refCuerpo;
  final List<Offset> refBase;
  
  final List<bool> cubiertosTecho;
  final List<bool> cubiertosCuerpo;
  final List<bool> cubiertosBase;
  ...
  _MontessoriPainter({
    ...
    required this.refTecho,
    required this.refCuerpo,
    required this.refBase,
    required this.cubiertosTecho,
    required this.cubiertosCuerpo,
    required this.cubiertosBase,
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final pathI = Path();
    pathI.moveTo(110, 110);
    pathI.lineTo(230, 110);
    pathI.moveTo(170, 110);
    pathI.lineTo(170, 340);
    pathI.moveTo(110, 340);
    pathI.lineTo(230, 340);

    canvas.drawPath(pathI, shadowPaint);
    canvas.drawPath(pathI, letterPaint);
    ...
    if (faseActual == 0) {
      for (int i = 0; i < refTecho.length; i++) {
        if (!cubiertosTecho[i]) canvas.drawCircle(refTecho[i], 5.0 + (factorParpadeo * 3), paintSemilla);
      }
    } else if (faseActual == 1) {
      for (int i = 0; i < refCuerpo.length; i++) {
        if (!cubiertosCuerpo[i]) canvas.drawCircle(refCuerpo[i], 5.0 + (factorParpadeo * 3), paintSemilla);
      }
    } else if (faseActual == 2) {
      for (int i = 0; i < refBase.length; i++) {
        if (!cubiertosBase[i]) canvas.drawCircle(refBase[i], 5.0 + (factorParpadeo * 3), paintSemilla);
      }
    }

    for (var papa in huerto) {
      if (papa.escalaActual > 0.05) {
        canvas.save();
        canvas.translate(papa.posicion.dx, papa.posicion.dy);
        canvas.rotate(papa.rotacion);
        canvas.scale(papa.escalaActual);
