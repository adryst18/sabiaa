...
class PapaAnimada {
  ...
  PapaAnimada({required this.posicion});
}
...
class _TrazoIMayusculaScreenState extends State<TrazoIMayusculaScreen> with TickerProviderStateMixin {
  ...
  void _generarReferencias() {
    _puntos.clear();
    for (double x = 110; x <= 230; x += 12) { _puntos.add(Offset(x, 110)); } // Techo
    for (double y = 122; y <= 330; y += 12) { _puntos.add(Offset(170, y)); } // Cuerpo
    for (double x = 110; x <= 230; x += 12) { _puntos.add(Offset(x, 340)); } // Base
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(backgroundColor: const Color(0xFF134074), title: const Text('SABIA - Letra I Mayúscula', style: TextStyle(color: Colors.white))),
      body: Column(
        children: [
          Expanded(
            child: Center(
                ...
                child: GestureDetector(
                  ...
                  onPanUpdate: (d) {
                    ...
                    setState(() {
                      ...
                      if (_trazoActual.length % 4 == 0) _huerto.add(PapaAnimada(posicion: d.localPosition));
                    });
                  },
                  onPanEnd: (_) {
                    _audioPlayer.stop();
                  ...
                  child: AnimatedBuilder(
                    ...
                    builder: (context, child) => CustomPaint(
                      painter: LetraIPainter(_trazos, _trazoActual, _huerto, _puntos, _cubiertos, _blink.value),
                      size: Size.infinite,
                    ),
                  ),
                ),
  ...
  void _mostrarDialogo() {
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('¡Magnífico!', textAlign: TextAlign.center, style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          Row(
            ...
            children: [
              ...
              IconButton(
                ...
                onPressed: () { Navigator.pop(context); Navigator.push(context, MaterialPageRoute(builder: (context) => const TrazoIMinusculaScreen())); },
...
class LetraIPainter extends CustomPainter {
  final List<List<Offset>> trazos; final List<Offset> trazoActual; final List<PapaAnimada> huerto;
  final List<Offset> puntos; final List<bool> cubiertos; final double blink;
  LetraIPainter(this.trazos, this.trazoActual, this.huerto, this.puntos, this.cubiertos, this.blink);
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final p = Path()..moveTo(110, 110)..lineTo(230, 110)..moveTo(170, 110)..lineTo(170, 340)..moveTo(110, 340)..lineTo(230, 340);
    canvas.drawPath(p, sPaint); canvas.drawPath(p, lPaint);
